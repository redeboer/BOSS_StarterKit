...
${CMTROOT}/mgr/cmt cleanup -sh -pack=cmt_standalone -version=v0 -path=${BOSS_StarterPack}/cmthome  $* >${cmtcmt_standalonetempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt cleanup -sh -pack=cmt_standalone -version=v0 -path=${BOSS_StarterPack}/cmthome  $* >${cmtcmt_standalonetempfile}"
  cmtcleanupstatus=2
