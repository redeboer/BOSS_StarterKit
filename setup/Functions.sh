...
function CreateBaseDir() { # Creates a base directory for a file to which you want to write.
  mkdir -p "$(dirname "${1}")"
}
export CreateBaseDir

function CdToBaseDir() { # Creates a base directory for a file to which you want to write.
  gStoredDirectory="$(pwd)"
  cd "$(dirname "${1}")"
}
export CdToBaseDir

function CreateOrEmptyDirectory() { # Create a directory if necessary. If it already exists, remove the already existing files (with a certain pattern).
...
function ResourceCommonFunctions() { # The parameter `gFunctionsScriptIsSourced` is set to `true` if this script is sourced. Use this function if you really want to bypass this safety measure and source again.
...
function AffirmMkdir() { # Check if the path to a folder exists. Exit the script if it doesn't.
...
function ChangeLineEndingsFromWindowsToUnix() { # Windows sometimes stores files with a different type of line endings. To make the file compatible again with Unix/Linux, use this function.
  # * Import function arguments
  local fileName=${1}
